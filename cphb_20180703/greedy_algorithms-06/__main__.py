...
허프만 코드 (huffman coding)는 욕심쟁이 알고리즘을 사용해 주어진 문자열을 효율적으로 압축하는 기법이다. 이 알고리즘은
...
허프만 코드의 첫 단계는 문자열의 각 문자를 노드로 나타내는 것이다. 이때 문자와 대응하는 노드의 무게는 그 문자가
문자열에서 나타나는 횟수로 정한다. 그 다음 단계에서는 최소 무게를 가진 두 개의 노드를 합쳐 무게가 노드 두 개의 무게를
합한 값인 새로운 노드를 만든다. 모든 노드가 합쳐질 때까지 이 작업은 계속 진행된다.
...
"""
