...
O(···)의 시간 복잡도에서 ···는 함수를 나타내고, 일반적으로 변수 n은 문자열의 길이나 배열의 크기 같은
입력값의 크기를 나타낸다.

1. 시간 복잡도 계산 방법
- 반복문이 k개 중첩되어 있을 때, 이 알고리즘의 시간 복잡도는 O(n^k)이다.ㄴ
...
일반적으로, 알고리즘의 시간 복잡도가 최악의 경우 O(n^k) (단, k는 상수)일 때, 이 알고리즘을 다항 시간 알고리즘
(polynomial algorithm)이라고 한다. O(2^n)과 O(n!)을 제외한 위 예시의 모든 알고리즘은 다항 시간 알고리즘이다.
대부분의 경우 상수 k는 아주 작기 때문에, 어떤 알고리즘이 다항 시간 복잡도를 가진다는 것은 곧 알고리즘이 효율적이라는
